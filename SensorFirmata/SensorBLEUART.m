...
@implementation SensorBLEUART


-(id)initWithPeripheral:(CBPeripheral *)device serviceUUID:(NSString *)serviceUUID readUUID:(NSString *)readUUID writeNotifyUUID:(NSString *)writeNotifyUUID {
    if (self = [super init]){
        self.pServiceUUID = serviceUUID;
        self.pReadUUID = readUUID;
        self.pNotifyUUID = writeNotifyUUID;
        self.device = device;
...
//    return [CBUUID UUIDWithString:@"FFF0"];
    return [CBUUID UUIDWithString:self.pServiceUUID];

}

-(CBUUID*)readUUID {
//    return [CBUUID UUIDWithString:@"FFF2"];
    return [CBUUID UUIDWithString:self.pReadUUID];
}
-(CBUUID*)writeNotifyUUID {
    
//    return [CBUUID UUIDWithString:@"FFF1"];
    return [CBUUID UUIDWithString:self.pNotifyUUID];
}
