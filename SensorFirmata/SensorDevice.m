...
+(SensorDevice*)shared{
    ...
    dispatch_once(&onceToken, ^{
        _sensor = [[SensorDevice alloc] init];
    });
    return _sensor;
}
...
-(SensorUART*)uart {
    ...
    if (self.connectedServices.count > 0){
        return (SensorUART*)[self.connectedServices lastObject];
    }
    return nil;
}
...
- (void) clearDevices
{
    ...
    for (SensorUART* service in self.connectedServices) {
        [service reset];
    }
...
-(void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    SensorUART   *service    = nil;
    
    /* Create a service instance. */
    service = [[SensorUART alloc] initWithPeripheral:peripheral serviceUUID:self.pServiceUUID readUUID:self.pReadUUID writeNotifyUUID:self.pNotifyUUID];
    [service start];
